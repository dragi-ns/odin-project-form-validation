...
function validateCountryInput(input) {
  ...
  const countryPostal = COUNTRY_ADDRESS_POSTALS.find(
    ({ name }) => name.toLowerCase() === inputValue
  );
  if (!countryPostal) {
    input.setCustomValidity('Country is invalid.');
  } else {
...
function validateZipInput(countryInput, zipInput) {
  const zipInputValue = zipInput.value.trim().toLowerCase();
  if (!zipInputValue) {
    return;
  }
  const countryInputValue = countryInput.value.trim().toLowerCase();
  const countryPostal = COUNTRY_ADDRESS_POSTALS.find(
    ({ name }) => name.toLowerCase() === countryInputValue
  );
  if (!countryPostal || zipInputValue.match(countryPostal.postal) === null) {
    zipInput.setCustomValidity(`Zip code is invalid!`);
  } else {
...
function validatePassword(passwordInput, passwordConfirmInput) {
...
function isFormValid() {
  return Array.from(form.elements).reduce((aggregator, input) => {
    if (input.type === 'submit') {
      return aggregator;
    }
    validateInput(input);
...
email.addEventListener('input', validateInput.bind(null, email));
email.addEventListener('blur', validateInput.bind(null, email));

country.addEventListener('input', validateCountryInput.bind(null, country));
country.addEventListener('blur', validateCountryInput.bind(null, country));

zip.addEventListener('input', validateZipInput.bind(null, country, zip));
zip.addEventListener('blur', validateZipInput.bind(null, country, zip));

password.addEventListener(
  'input',
  validatePassword.bind(null, password, passwordConfirm)
);
password.addEventListener(
  'blur',
  validatePassword.bind(null, password, passwordConfirm)
